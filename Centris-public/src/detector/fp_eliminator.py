...
verIDXpath = currentPath + "/../preprocessor/verIDX/"  # Default path
initialDBPath = currentPath + "/../preprocessor/initialSigs/"  # Default path
finalDBPath = currentPath + "/../preprocessor/componentDB_fpelm/"  # Default path of the final Component DB
metaPath = currentPath + "/../preprocessor/metaInfos/"  # Default path, for saving pieces of meta-information of collected repositories
weightPath = metaPath + "/weights/"  # Default path, for version prediction
funcDatePath = currentPath + "/../preprocessor/funcDate/"  # Default path
            ...
            for candidate_file_1 in candidate_file_and_func_1.keys():
                for candidate_file_2 in candidate_file_and_func_2.keys():
                    if candidate_file_1 == candidate_file_2:
                        ...
                        compare_res = compare_hash_sets(candidate_func_set_1, candidate_func_set_2, candidate_name_1, candidate_name_2, verDateDict)
                        if compare_res and compare_res[0]:
                            ...
                            if key not in fp_elimination_res:
                                fp_elimination_res[key] = set()
                            fp_elimination_res[key].add(candidate_file_1+" "+str(compare_res[1][0]) +" "+str(compare_res[1][1]) + " " + str(belongs_to))
