...
clonePath = "/home/shangzhixu/IoTDB/"  # Default path
...
mappingPath = currentPath + "/repo_mapping/"  # Default path

# Generate directories
shouldMake = [clonePath, tagDatePath, resultPath, mappingPath]
...
def hashing(repoPath, tag):
                ...
                try:
                    ...
                    for i in allFuncs:
                        ...
                        if i != '' and len(elemList) >= 8 and func.fullmatch(elemList[3]):
                            ...
                            funcInfo = funcName + "@" + str(funcStartLine) + "@" + str(funcEndLine) +  "@" + str(tag)
                            hashingFuncMapping[funcHash] = funcInfo
    ...
    print ("[+] Function Count:", funcCnt)
    print ("[+] Line Count:", lineCnt)
    # printout resDict
    # for hashval in hashingFuncMapping:
    #    print(hashval, hashingFuncMapping[hashval])
    return resDict, fileCnt, funcCnt, lineCnt, hashingFuncMapping


def indexing(resDict, title, filePath ,  hashingFuncMapping):
    ...
    for hashval in resDict:
        ...
        for funcPath in resDict[hashval]:
            fres.write('\t' + funcPath)
        if hashingFuncMapping[hashval] != "":
            funcInfo = hashingFuncMapping[hashval]
            fres.write('\t' + funcInfo)
        fres.write('\n')
        ...
        for eachUrl in lines:
            ...
            try:
                ...
                if tagResult == "":
                    ...
                    # print("No tags")
                    resDict, fileCnt, funcCnt, lineCnt,  hashingFuncMapping = hashing(clonePath + repoName, None)
                    if len(resDict) > 0:
                        ...
                        indexing(resDict, title, resultFilePath,  hashingFuncMapping)
                    ...
                    for tag in str(tagResult).split('\n'):
                        ...
                        checkoutCommand = subprocess.check_output("git checkout -f " + tag, stderr=subprocess.STDOUT,
                                                                  shell=True)
                        resDict, fileCnt, funcCnt, lineCnt,  hashingFuncMapping = hashing(clonePath + repoName, tag)
            ...
            end_time = time.time()
            print(f"{repoName} finished in {end_time - start_time} seconds")
            with open("/home/shangzhixu/Centris-public-IoTTest/src/osscollector/time.txt", "a") as f:
                f.write(f"{repoName} : {end_time - start_time}\n")
