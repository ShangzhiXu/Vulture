...
resultPath		= "../osscollector/repo_functions_3000test/" 		# Default path
verIDXpath		= currentPath + "/verIDX_3000full/"					# Default path
initialDBPath	= currentPath + "/initialSigs_3000full/"  			# Default path
finalDBPath		= currentPath + "/componentDB_3000full/"  			# Default path of the final Component DB
metaPath		= currentPath + "/metaInfos_3000full/"				# Default path, for saving pieces of meta-information of collected repositories
weightPath		= metaPath 	  + "/weights_3000full/"					# Default path, for version prediction
funcDatePath	= currentPath + "/funcDate_3000full/"				# Default path
...
def codeSegmentation():
    ...
    for S_sig in OSSList:
        ...
        # walk through the subdirs in finalDBPath
        hasVisited = False
        for filename in os.listdir(finalDBPath):
            if os.path.isfile(os.path.join(finalDBPath, filename)):
                # get the file name
                #print("filename: ", filename, "S_sig: ", S_sig)
                if filename == S_sig:
                    hasVisited = True
                    break
        if hasVisited:
            print("Already visited: ", S_sig)
            l += 1
            #continue
        ...
        with open(initialDBPath + S_sig, 'r', encoding = "UTF-8") as fs:
            ...
            if len(jsonStr) == 0:
                ...
                # if S not in possibleMembers, then copy the signature to finalDBPath, nothing to remove
                if S not in possibleMembers:
                    try:
                        shutil.copy(os.path.join(initialDBPath, S) + "_sig", os.path.join(finalDBPath, S) + "_sig")
                    except:
                        print("Error at line ", l, " in codeSegmentation")
                        continue

                else:
                    removedFuncs = set(removedFuncs)
                    saveJson = []
                    with open("./3000test/" + "removedFuncNumFull", 'a') as fres:
                        fres.write(S + "\t" + str(len(removedFuncs)) + '\t' + str(len(possibleMembers[S])) + '\n')
