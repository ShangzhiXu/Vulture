...
initialDBPath = currentPath + "/componentDB/"  # Default path
finalDBPath = currentPath + "/componentDB_fpelm /"  # Default path of the final Component DB
...
# extract the date and tag as string
...
def redundancyElimination():
    for repoName in os.listdir(resultPath):
        if repoName not in need_process:
            #print(f"{repoName} has been processed")
            continue
        # visit ./componentDB list all the files in the directory
        # if the file is already in the directory, skip it
        # visited_files_incomponentdb = []
        # for path, dir, files in os.walk("./componentDB"):
        #     for file in files:
        #         visited_files_incomponentdb.append(file)
        # if repoName + "_sig" in visited_files_incomponentdb:
        #     print(f"{repoName} has been processed")
        #     continue
        ...
        for hashval in tempDateDict:
            ...
            # get the first date of the function creation
...
def saveMetaInfos():
    ...
    for OSS in os.listdir(initialDBPath):
        ...
        try:
            ...
            print("Error at saveMetaInfos: ", repoName, e)
            continue
...
def codeSegmentation():
    ...
    for S_sig in OSSList:
        ...
        # S is the same "S" in the paper algorithm 1
        ...
        file_size = os.path.getsize(initialDBPath + S_sig)
        # print("file size is : ", file_size)
        # if (file_size >= 3000000):
        #     with open("large_file.txt", 'a') as f:
        #         f.write(S_sig + "\n")
        #     continue
        with open(initialDBPath + S_sig, 'r', encoding="UTF-8") as fs:
            ...
            if len(jsonStr) == 0:
                ...
                # temp is a dict to store the possible members of S
                temp = {}
                for eachVal in jsonStr:
                    #get the hash value of the function in each repo, like each function in libtuv
                    hashval = eachVal['hash']

                    for OSS in uniqueFuncs[hashval]:
                        # uniqueFuncs[hashval] is the list of repo names that contain the function
                        if OSS == S:
                            continue
                        ...
                        try:
                            ...
                            pass
                #here the X is the "members" in the paper algorithm 1
                for X in candiX:
                    if aveFuncs[X] == 0:
                        continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('process_id_str', type=str, help='process id string')
    ...
    #global verIDXpath
    verIDXpath = currentPath + "/verIDX" + process_id_str + "/"  # Default path

    #global initialDBPath
    initialDBPath = currentPath + "/initialSigs" + process_id_str + "/"  # Default path

    #global metaPath
    metaPath = currentPath + "/metaInfos" + process_id_str + "/"  # Default path, for saving pieces of meta-information of collected repositories

    #global weightPath
    weightPath = metaPath + "/weights" + process_id_str + "/"  # Default path, for version prediction

    #global funcDatePath
    funcDatePath = currentPath + "/funcDate" + process_id_str + "/"  # Default path
    ...
    with open(target_file, 'r', encoding="UTF-8") as fp:
        for item in fp.readlines():
            need_process.append(item.strip())
