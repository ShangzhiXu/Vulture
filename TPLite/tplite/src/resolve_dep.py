...
def main():
    ...
    for data in df.itertuples():
        try:
            ...
            logger.info(f"Error of tpl name: {data[1]}")
            continue

    tpl_sigs_path = store_path.joinpath("tpl_sigs.pkl")
    # record time, path of function
    tpl_sigs = obtain_tpl_sigs()

    func_info_path = store_path.joinpath("func_info_all.pkl")
    # just copy the info from the output/x.json to func_info_all.pkl
    # recode the first commit time of each function & function hash
